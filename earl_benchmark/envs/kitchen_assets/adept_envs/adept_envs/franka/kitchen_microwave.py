...
class KitchenV0(robot_env.RobotEnv):
    ...
    def __init__(self, robot_params={}, frame_skip=40):
        ...
        self.mocap_pos_clip_lower = np.array([-0.7, -0.1, 1.8])
    ...
    def step(self, a, b=None):
        ...
        new_pos = current_pos + a[:3]*self.range
    ...
    def _get_obs(self):
        ...
        self.obs_dict['mocap_pos'] = self.sim.data.mocap_pos.copy().squeeze(axis=0)
        return np.concatenate([self.obs_dict['qp'], self.obs_dict['obj_qp'], self.obs_dict['mocap_pos']])
...
class KitchenMicrowaveV0(KitchenV0):
    ...
    def __init__(self):
        super(KitchenMicrowaveV0, self).__init__()
    ...
    def _get_reward_n_score(self, obs_dict):
        ...
        score = 0.
        target_pos = self.sim.data.site_xpos[self.sim.model.site_name2id('microhandle_site')]
        ee_error = target_pos - obs_dict['mocap_pos']
        slider_error = obs_dict['obj_qp'][13:14] - (-1.0)
        reward_dict['r_total'] = -1 * np.linalg.norm(ee_error) -10 * np.linalg.norm(slider_error)
        reward_dict['true_reward'] = reward_dict['true_reward']
    ...
    def render(self, mode='human'):
        if mode =='rgb_array':
            ...
            super(KitchenMicrowaveV0, self).render()
